...
def test_take_in_a_list_with_same_format_and_the_file_exists_with_correct_json_format():
    test_list = [{"name": "It takes two", "required storage": 70}, {"name": "God of War", "required storage": 70}]
    result = json_convertor.read_and_load("test_file_correct_json_format.txt",test_list)
    assert result == True

def test_take_in_a_list_with_different_format_and_the_file_exists_with_correct_json_format():
    test_list = ["apple","orange","banana"]
    result = json_convertor.read_and_load("test_file_correct_json_format.txt",test_list)
    assert result == True